...
export const FNO_SYMBOLS = [
  { symbol:"NIFTY",      token:"120000", exchange:"NSE" },
  { symbol:"BANKNIFTY",  token:"120000", exchange:"NSE" },
  { symbol:"RELIANCE",   token:"2885",  exchange:"NSE" },
  { symbol:"TCS",        token:"120000", exchange:"NSE" },
  ...
  { symbol:"BHARTIARTL", token:"120000", exchange:"NSE" },
...
export function connectAngelOneFeed(engines, log) {
  ...
  ws.on("message", (raw) => {
    try {
      if (raw instanceof Buffer) {
        ...
        const ltp = raw.readUInt32BE(35) / 100;
  ...
  ws.on("close", () => { log.warn("Angel One feed WS closed — reconnecting in 5s..."); setTimeout(() => connectAngelOneFeed(engines, log), 5000); });
