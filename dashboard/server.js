...
import { niftyMtfRefreshStatusResult } from './routes/nifty-mtf-refresh-status.js';
  ...
  try {
    ...
    if (!handler) {
      sendJson(response, 404, {
        error: 'NOT_FOUND',
        message: 'Supported endpoints: /api/health, /api/status, /api/manifest, /api/daily, /api/intraday, /api/governance, /api/admission, /api/nifty-mtf, /api/nifty-mtf/refresh-status'
      });
      return;
    }
